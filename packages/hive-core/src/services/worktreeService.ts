  ...
  async create(feature: string, step: string, baseBranch?: string): Promise<WorktreeInfo> {
    ...
    if (baseBranch) {
      ...
      try {
        ...
        // Diagnostic: capture exact state at failure time
        const fsSync = await import("fs");
        const { execSync } = await import("child_process");
        let gitVersion = "unknown";
        let gitConfig = "unknown";
        try { gitVersion = execSync("git --version", { encoding: "utf-8" }).trim(); } catch {}
        try { gitConfig = execSync("git config --global --list", { encoding: "utf-8" }).trim(); } catch {}
